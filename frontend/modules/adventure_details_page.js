...
async function fetchAdventureDetails(adventureId) {
  ...
  try{
    let a = await fetch("http://3.108.240.231:8082/adventures/detail?adventure="+`${adventureId}`)
...
function addAdventureDetailsToDOM(adventure) {
  ...
  let adventureN = document.getElementById("adventure-name")
 adventureN.append(adventure.name)
 ...
 content.append(adventure.content)
 
 adventure.images.forEach((e)=>{
  let pic = document.getElementById("photo-gallery")
  pic.innerHTML += `<div class="activity-card-image"><img src="${e}"</div>`
 })
...
function addBootstrapPhotoGallery(images) {
  ...
  <div class="carousel-inner" id="boot">
  ...
  images.forEach((ind,i)=>{
   let bstrap= document.getElementById("boot")
   bstrap.innerHTML+= `<div class= "carousel-item activity-card-image ${
    i === 0 ? " active" : ""
  }">
<img src="${ind}" class="d-block w-100">
</div>`;
  })
...
function captureFormSubmit(adventure) {
  // TODO: MODULE_RESERVATIONS
  let res = document.getElementById("myForm")
